...
// Test endpoint for insurance status emails
export async function GET() {
  try {
    ...
    if (emailsSent === 0) {
      const fallbackEmail = process.env.SMTP_FROM || 'credentialing@allstarbillingservice.com';
      try {
        await transporter.sendMail({
          from: fallbackEmail,
          to: fallbackEmail,
          subject: 'Weekly Insurance Application Status Update - Test Email',
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              <div style="background: #800020; color: white; padding: 20px; text-align: center;">
                <h1 style="margin: 0;">AllStar Billing Service</h1>
                <p style="margin: 5px 0 0 0;">Insurance Application Status Update (Test)</p>
              </div>
              <div style="padding: 30px; background: #f9f9f9;">
                <p>No provider emails were sent because there are no providers with insurance applications and valid email addresses.</p>
                <p><strong>Providers with applications:</strong> ${providers.length}</p>
                <p>If you expect emails to go out, add at least one insurance application for a provider with an email address.</p>
              </div>
            </div>
          `,
        });
        ...
        console.error('Error sending fallback test email:', emailError);
      }
    }
    ...
    return NextResponse.json(
      {
        ...
        emailsSent,
        providersWithApplications: providers.length,
        fallbackSent,
